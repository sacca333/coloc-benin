...
export default function AdminPage() {
  ...
  useEffect(() => {
    Promise.all([api.get('/admin/stats'), api.get('/admin/activite')])
      .then(([s, a]) => { setStats(s.data); setActivite(a.data); });
  }, []);

  if (!stats) return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh' }}>
      <div style={{ width: 36, height: 36, border: '2px solid #10B981', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin .8s linear infinite' }} />
    </div>
  );
