...
const photoUrl = (path: string) => `${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}${path}`;
    ...
    return (
        <div style={{ maxWidth: 1160, fontFamily: "'Syne',sans-serif" }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
                <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Ville, description..."
                    style={{ flex: 1, background: '#080F1C', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '9px 14px', color: '#F1F5F9', fontSize: 13, outline: 'none', fontFamily: "'Syne',sans-serif" }} />
                <select value={statut} onChange={e => { setStatut(e.target.value); setPage(1); }}
                    style={{ background: '#080F1C', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '9px 14px', color: '#F1F5F9', fontSize: 13, outline: 'none', cursor: 'pointer' }}>
                                ...
                                <tr key={a.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background .15s' }}
                                    ...
                                    <td style={{ padding: '10px 14px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                                            {a.photos?.[0] ? <img src={photoUrl(a.photos[0])} alt="" style={{ width: 38, height: 38, borderRadius: 8, objectFit: 'cover' }} /> : (
