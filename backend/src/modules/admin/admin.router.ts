...
adminRouter.get('/activite', async (_req, res) => {
  try {
    const [recentUsers, recentAnnonces, recentAbonnements] = await Promise.all([
      prisma.utilisateur.findMany({ take: 5, orderBy: { createdAt: 'desc' }, select: { id: true, nom: true, prenom: true, email: true, photo: true, createdAt: true, typeCompte: true } }),
      prisma.annonce.findMany({ take: 5, orderBy: { createdAt: 'desc' }, include: { proprietaire: { select: { nom: true, prenom: true } } } }),
